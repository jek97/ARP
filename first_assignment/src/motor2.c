...
int main(int argc, char const *argv[]) {
    ...
    while(1){
        if(read(Vz_m2, Vz_rcv, sizeof(Vz_rcv)) < 0) {
        ...
        else if (read(Vz_m2, Vz_rcv, sizeof(Vz_rcv)) == 0) {
            perror("no proces has opened the Vz pipe for writing"); // checking errors
        }
        else {
            if (z_i >= 0 && z_i <= 40){
            ...
            else if (z_i > 40) { // reached upper bound stop at that position
                z_i = 40;
            }
